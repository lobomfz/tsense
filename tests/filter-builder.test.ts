...
describe("presets", () => {
  it("includes presets in descriptor", () => {
    const builder = createFilterBuilder(UsersCollection, {
      age: {
        label: "Age",
    ...
    const descriptor = builder.describe();
    const age = descriptor.columns[0];

    expect(age.presets).toBeDefined();
    expect(age.presets!.length).toBe(1);
    expect(age.presets![0].name).toBe("Over 18");
    expect(age.presets![0].filter).toEqual({ age: { gte: 18 } });
  });
  ...
  it("includes multiple presets", () => {
    ...
    const descriptor = builder.describe();
    const names = descriptor.columns[0].presets!.map((p) => p.name);

    expect(names).toContain("Over 18");
    expect(names).toContain("Under 30");
  });

  it("does not include presets when not configured", () => {
    const builder = createFilterBuilder(UsersCollection, {
      age: { label: "Age" },
    });
    ...
    expect(descriptor.columns[0].presets).toBeUndefined();
  });
});
