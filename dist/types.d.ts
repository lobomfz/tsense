...
type FilterValueFor<T> = [T] extends [boolean] ? boolean | BooleanFilter : [T] extends [Date] ? DateValue | DateValue[] | DateFilter : [T] extends [number] ? number | number[] | NumberFilter : [T] extends [string] ? T | T[] | StringFilter : never;
...
export type SyncConfig<T> = {
    ...
    getItems: (ids: string[]) => Promise<(WithNull<T> & {
        id: string;
    })[]>;
    chunkSize?: number;
};
